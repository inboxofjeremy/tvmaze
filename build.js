...
// Lookup via TVMaze IMDB endpoint
...
// 2) episodesByDate fallback (for missing shows)
for (let i = 0; i < 10; i++) {
const d = new Date(todayStr);
d.setDate(d.getDate() - i);
...
const url = `https://api.tvmaze.com/schedule?date=${dateStr}`;
const generic = await fetchJSON(url);
...
// 3) TMDB fallback for missing shows
for (const [id, info] of showMap.entries()) {
if (!info.show.externals?.imdb) continue;
